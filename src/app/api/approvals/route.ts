...
import { lifeGridRunner, LIFEGRID_USER_ID } from '@/lib/adk/runner';
  ...
  try {
    ...
    if (sessionId && Array.isArray(decisions) && decisions.length > 0) {
      ...
      const events = lifeGridRunner.runAsync({
        ...
        newMessage: {
          role: 'user',
          parts: decisions.map((d: { functionCallId: string; action: 'approve' | 'reject' }) =>
            createPartFromFunctionResponse(d.functionCallId, 'adk_request_confirmation', { confirmed: d.action === 'approve' })
          ),
        },
      });
